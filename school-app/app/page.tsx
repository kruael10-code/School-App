...
const DetailModal: React.FC<DetailModalProps> = ({ isOpen, onClose, data }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Create a hidden clone container to simulate desktop width
    const cloneContainer = document.createElement('div');
    cloneContainer.style.position = 'fixed';
    cloneContainer.style.top = '-9999px';
    cloneContainer.style.left = '-9999px';
    cloneContainer.style.width = '800px'; // Force A4 Width
    ...
    try {
      // Longer delay for mobile rendering
      ...
      alert("ไม่สามารถสร้าง PDF ได้ในขณะนี้ กรุณาลองใหม่: " + (error as Error).message);
    } finally {
      // Cleanup
